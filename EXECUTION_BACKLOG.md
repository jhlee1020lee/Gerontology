...
| DG-09 | P1 | Mark `Plan.md` as legacy / archival / non-authoritative at the top. | It conflicts with one-reading-at-a-time policy. | DG-01 | First lines clearly mark it non-normative. | `1 header pass` |
| DG-10 | P1 | Mark `Status.md` as historical log only and annotate legacy thresholds. | Historical rules are too easy to misread as current rules. | DG-01 | Old thresholds are clearly labeled as historical snapshots. | `1 header + 1 note pass` |
