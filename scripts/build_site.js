...
function isAccessibleReading(reading){if(reading.workflow_status===READING_STATUS.BLOCKED)return false;return isPublishedReading(reading)||isApprovedStatus(landingStatus(reading))||hasApprovedReadingPage(reading);}
...
`;}
function buildProfessorPrep(siteMeta,reading,page){const outputPath=path.join(siteDir,"readings",reading.slug,page.filename);const prep=loadProfessorPrep(page.sourcePath);const deck=prep?buildProfessorPrepDeck(prep):null;const content=!isAccessibleReading(reading)?pendingReadingHtml(reading,page.label):!page.review_passed?pendingUploadHtml(reading,page.label):prep?`
