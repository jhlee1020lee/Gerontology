...
function revealSummaryLabel(reveal){
  if(reveal?.unit==="sentence_group")return "이 문단과 대응하는 원문 일부 보기";
  if(reveal?.unit==="context_block")return "이 문단이 참고한 원문 단락 보기";
  return "이 문단에 대응하는 원문 보기";
}
...
function chapterNumberLabel(reading){
  ...
  const match=String(reading.source_filename||"").match(/\[?CH\s*0*(\d+)\]?/i);
  if(match)return `노년학 CH${Number(match[1])}`;
  return "노년학 교재";
}
...
function isAccessibleReading(reading){if(isBlockedReading(reading))return false;return isPublishedReading(reading);}
...
function canRenderPageContent(reading,page){return !isBlockedReading(reading)&&isSourceApprovedPage(page);}
...
`;}
function renderGatedTab(label,active=false){return `<button class="tab${active?" active":""} is-locked" type="button"${active?' aria-current="page"':""} aria-disabled="true" data-tab-link data-gated-link data-gated-message="준비중입니다.">${escapeHtml(label)}</button>`;}
...
function pageTabs(outputPath,reading,activeKey){const accessible=isAccessibleReading(reading);const blocked=isBlockedReading(reading);const base=path.join(siteDir,"readings",reading.slug);const tabs=[{key:"index",label:LANDING_TAB_LABEL,target:path.join(base,"index.html"),status:landingStatus(reading),sourceStatus:landingStatus(reading)}].concat(reading.pages.map((page)=>({key:page.key,label:page.label,target:path.join(base,page.filename),status:page.validation_status,sourceStatus:page.source_validation_status})));const primaryKeys=new Set(["index","full","translation","professor-prep"]);const primaryTabs=tabs.filter((tab)=>primaryKeys.has(tab.key));const hiddenTabs=tabs.filter((tab)=>!primaryKeys.has(tab.key));const hiddenActive=hiddenTabs.find((tab)=>tab.key===activeKey)||null;const renderTab=(tab)=>{if(blocked)return renderGatedTab(tab.label,tab.key===activeKey);if(tab.key==="index")return accessible?renderActiveTab(outputPath,tab,tab.key===activeKey):renderGatedTab(tab.label,tab.key===activeKey);return isApprovedStatus(tab.sourceStatus)?renderActiveTab(outputPath,tab,tab.key===activeKey):renderGatedTab(tab.label,tab.key===activeKey);};const hiddenMarkup=!hiddenTabs.length?"":`<details class="tab-more${hiddenActive?" has-active":""}" data-tab-more><summary class="tab-more-toggle" data-tab-more-toggle>더보기</summary><div class="tab-more-list" data-tab-more-list>${hiddenTabs.map((tab)=>renderTab(tab).replace("data-tab-link","data-tab-link data-tab-more-link")).join("")}</div></details>`;return `<nav class="tab-row" data-tab-row>${primaryTabs.map((tab)=>renderTab(tab)).join("")}${hiddenMarkup}</nav>`;}
...
function buildIndex(siteMeta,readings,thumbnails){const outputPath=path.join(siteDir,"index.html");const chatbotConfigAssetHref=relHref(outputPath,path.join(siteDir,"assets","chatbot-config.js"));const chatbotCorpusAssetHref=chatbotCorpusHref(outputPath);const sortedReadings=[...readings].sort((a,b)=>compareReadings(a,b,"chronological"));const cards=sortedReadings.map((reading,index)=>{const thumbHref=relHref(outputPath,path.join(siteDir,thumbnails[reading.slug]));const entryTarget=readingEntryTarget(reading);const targetHref=entryTarget?relHref(outputPath,path.join(siteDir,"readings",reading.slug,entryTarget)):"";const metaBits=cardMetaLabel(reading);const cardAuthorText=cardAuthorLabel(reading);const authorLine=cardAuthorText?`<p class="meta card-authors">${escapeHtml(cardAuthorText)}</p>`:`<p class="meta card-authors is-empty" aria-hidden="true">&nbsp;</p>`;const cardOpen=Boolean(entryTarget);const openTag=cardOpen?`<a class="card-link" href="${escapeHtml(targetHref)}">`:`<button class="card-link is-locked" type="button" data-gated-link data-gated-message="준비중입니다.">`;const closeTag=cardOpen?"</a>":"</button>";const stateClass=cardOpen?"ready":"locked";const stateLabel=cardOpen?"바로 보기":"준비 중";return `
  ...
  ${openTag}
    ...
    <div class="card-body">
      ...
      <div class="card-footer">
        <p class="card-state ${stateClass}">${stateLabel}</p>
      </div>
    </div>
  ${closeTag}
</article>
...
<main class="home-shell home-grid-shell" data-page-kind="home">
  <section class="home-guide panel">
    <h1>주차별 학습 일정에 따라 자료를 열람해 주세요.</h1>
    <p class="meta">활성화된 카드는 즉시 확인 가능하며, 준비 중인 자료는 잠금 상태로 표시됩니다.</p>
  </section>
...
function buildLanding(siteMeta,reading){const outputPath=path.join(siteDir,"readings",reading.slug,"index.html");const accessible=isAccessibleReading(reading);const blocked=isBlockedReading(reading);const pendingVideoMessage=toText(reading.notebooklm_video_note)||"설명 영상은 아직 준비 중이지만, 승인된 페이지는 탭에서 바로 열 수 있습니다.";const content=accessible&&isApprovedStatus(landingStatus(reading))?`<section class="video-stage"><div class="video-frame">${renderNotebookLmVideo(outputPath,reading)}</div></section>`:blocked?pendingReadingHtml(reading,LANDING_TAB_LABEL,{backHref:"../../index.html",backLabel:"홈으로 돌아가기"}):pendingUploadHtml(reading,LANDING_TAB_LABEL,pendingVideoMessage,{lead:"설명 영상은 아직 업로드되지 않았습니다.",backHref:"../../index.html",backLabel:"홈으로 돌아가기"});const body=`
${siteHeader(siteMeta,outputPath)}
...
function refreshReadings(manifest,slugFilter=null){ensureContentPlaceholders(manifest,slugFilter);return prepareReadings(manifest);}
